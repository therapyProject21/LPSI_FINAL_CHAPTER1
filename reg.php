<?php
// Include file koneksi (pastikan file transfer.php ada dan berfungsi dengan baik)
include 'transfer.php';

// Menampilkan error untuk debugging (pastikan hanya diaktifkan pada pengembangan)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            // Menampilkan error jika query tidak berhasil
            $errorInfo = $stmt->errorInfo();
            echo "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>
                    <strong>Error!</strong> Terjadi kesalahan: " . $errorInfo[2] . "
                  </div>";
        }
    } catch (PDOException $e) {
        // Menangkap dan menampilkan error jika terjadi kesalahan dalam query
        echo "<div class='bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4' role='alert'>
                <strong>Error!</strong> Terjadi kesalahan: " . $e->getMessage() . "
              </div>";
    }
